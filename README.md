...
**`UTDWAbility_LeapSlam`** is a Gameplay Ability (GAS) that launches the owning character toward a cursor-targeted location, then detonates a radial slam on landing. It is part of the **TheDarkWestTest** plugin and inherits from `UTDWAbility_Attack`.
...
ActivateAbility()
  ...
  |    +-- Clamps target inside [MinLeapRange, MaxLeapRange]
...
| `MinLeapRange` | `float` | `100` | Minimum 2D distance (cm) required for the leap to execute. Clicks closer than this cancel the ability. |
| `MaxLeapRange` | `float` | `1500` | Maximum 2D distance (cm). Targets beyond this are clamped to this range along the same direction. |
...
4. Set **SlamRadius**, **MinLeapRange**, **MaxLeapRange**, **KnockbackStrength** as needed.
